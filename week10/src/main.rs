...
fn problem_2() {
    ...
    {
        let r1 = &s; // immutable borrow in its own scope
        println!("  r1 = {}", r1);
...
fn problem_4() {
    ...
    {
        let r1 = &mut s; // first mutable borrow scoped
        println!("  r1 = {}", r1);
    } // r1 goes out of scope here
...
fn problem_6() {
    ...
    for i in 0..3 {
        print_with_number(&data, i); // pass a reference instead of moving
    }
}
...
fn problem_7() {
    ...
    let result = &s; // `s` lives for the whole function, so this reference is valid
